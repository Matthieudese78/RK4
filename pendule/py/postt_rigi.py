...
# %% indexes :
n_rigi = df[(df["rigi"]==1)]['n'].drop_duplicates().values 
ind_rigi = [ df[(df["n"]==ni) & (df["rigi"]==1) & (df["inert"]==0)].index for ni in n_rigi]
ind_rigi_inert = [ df[(df["n"]==ni) & (df["rigi"]==1) & (df["inert"]==1)].index for ni in n_rigi ]

k1 = {
    "col1" : "uzp2" ,
    "ind1" : ind_rigi_inert[1],
    "ind2" : ind_rigi[1],
    "col3" : "uzp2rigirela",
}
traj.relaind(df,**k1)
...
#%%  :
n = 6
for i,ni in enumerate(n_rigi): 
    kwargs1 = {
        "tile1": f"zp2 = f(t),"+r"$h = 10^{-%d}$" % ni + "\n",
        "tile_save": f"zp2_ft_n_{ni}",
        "colx": "t",
        "coly": "uzp2",
        "ind" : [ind_rigi[0],ind_rigi_inert[0]],
        "rep_save": repsect1,
        "label1": [r"$h = 10^{-%d}$" % ni] * 2,
        "labelx": r"$t \quad (s)$",
        "labely": r"$z \quad (m)$",
        "color1": color1,
        "loc_leg": (1.01,0.),
