...
for i,thi in enumerate(thinc): 
  reploadi = f'{repload}xi_{int(100.*xi)}/thini_{int(thi)}/nmode_{nmode}/'
  df = pd.read_pickle(f"{reploadi}result.pickle")
  ...
  # duree du choc global :
  tchocglob =  lst[-1] - fst[0]
  # nbr de muimpact ;
  nbchoc = len(fst) 
  # duree min/max d'une microimpact :
  tchocmin = np.min(tchoc)
  tchocmax = np.max(tchoc)
  lglob.append(tchocglob)
  lnbc.append(nbchoc)
  lmin.append(tchocmin)
  lmax.append(tchocmax)
#%%
plt.scatter(thinc,)
#%% energie potentielle :
muk = 0.075
